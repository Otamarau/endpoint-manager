...
3. Generate a salted passcode hash:

   ```sh
   npm run hash-passcode
   ```

   Copy the generated value into `.env` as `SITE_PASSCODE_HASH`.

4. Copy `.env.example` to `.env` and edit the settings for your environment:
...
5. Generate a self-signed HTTPS certificate (PowerShell 7 or newer):
...
7. Open `https://localhost:3000` and enter the configured site passcode. Your
...
| --- | --- | --- |
| `SITE_PASSCODE_HASH` | Yes | Salted scrypt verifier generated by `npm run hash-passcode`. |
...
- **`SITE_PASSCODE_HASH is not configured correctly`**: Run
  `npm run hash-passcode`, add its output to `.env`, then restart the server.
